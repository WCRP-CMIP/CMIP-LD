...
def mock_json_load(file):
    ...
    return {
        'CV': {
            ...
            'sub_experiment_id': ['none'],
            'realization_index': ['r1', 'r2'],
            'initialization_index': ['i1', 'i2'],
            'physics_index': ['p1', 'p2'],
            'forcing_index': ['f1', 'f2'],
            'institution_id': {'TEST': 'Test Institution'},
            'license': ["^CMIP6Plus model data produced by ..."],
            'nominal_resolution': ["0.5 km", "1 km", "10 km", "100 km", "1000 km"],
            'realm': {
                'aerosol': 'Aerosol',
                'atmos': 'Atmosphere',
                'ocnBgchem': 'Ocean Biogeochemistry',
            },
            'required_global_attributes': ["Conventions", "activity_id", "creation_date"],
            'source_id': {
                'GISS-E2-1-G': {
            ...
            'version_metadata': {'version_number': '1.0'},
            'product': 'model-output'
        }
    }

@pytest.fixture(scope="module")
def cv_data(monkeypatch):
    """Fixture to provide CV data for all tests."""
...
def test_cv_structure(cv_data):
    ...
    assert set(cv_data.keys()) == {'CV'}, "CV data should have only one top-level key: 'CV'"

def test_conventions(cv_data):
    """Test if the Conventions field is correctly formatted."""
    conventions = cv_data['CV']['Conventions']
    assert conventions == ["^CF-1.7 CMIP-6.[0-2,5]\\( UGRID-1.0\\)\\{0,\\}$"], f"Unexpected Conventions format: {conventions}"

def test_activity_id(cv_data):
    """Test if activity_id is a dictionary."""
    assert isinstance(cv_data['CV']['activity_id'], dict), "activity_id should be a dictionary"
...
def test_license(cv_data):
    ...
    assert isinstance(cv_data['CV']['license'], list), "license should be a list"
...
def test_realm(cv_data):
    ...
    expected_realms = {'aerosol', 'atmos', 'atmosChem', 'land', 'landIce', 'ocean', 'ocnBgchem', 'seaIce'}
...
def test_miscellaneous(cv_data):
    ...
    for field in ['table_id', 'tracking_id', 'variant_label']:
        assert isinstance(cv_data['CV'].get(field), list), f"{field} should be a list"
    assert isinstance(cv_data['CV']['version_metadata'], dict), "version_metadata should be a dictionary"
